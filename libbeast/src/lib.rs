...
impl BeastBuffer {
    ...
    pub fn read_single_frame(&mut self) -> Result<(), Error> {
    ...
    pub fn read_beast_buffer(&mut self) -> Result<(), Error> {
        ...
        if !msg.is_empty() {
            ...
            self.buffer = std::iter::once(0x1A).chain(reminder).collect();
        } else {
            self.buffer.clear();
        }
...
/// # Arguments
///
/// * `msg` - A Vec<u8> that holds the message.
///
/// # Returns
///
/// * The decoded Downlink Format value as an integer.
fn df(msg: &str) -> u32 {
    let dfbin = hex2bin(&msg[..2]);
    std::cmp::min(bin2int(&dfbin[..5]), 24)
}
...
/// # Arguments
///
/// * `bin` - A string slice that holds the binary string.
///
/// # Returns
///
/// * The integer representation of the binary string.
fn bin2int(bin: &str) -> u32 {
